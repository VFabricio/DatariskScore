...
let configureWebHostBuilder (builder: IWebHostBuilder) = builder.UseTestServer() |> ignore

let createHost () =
    let app =
        Config.initializeForTest () |> Program.createApp
...
let postCpfNotJson =
    task {
        ...
        Expect.equal response.StatusCode HttpStatusCode.BadRequest "a POST with a non-JSON body should return 400"
    }
...
let postCpfNoCpf =
    task {
        ...
        Expect.equal response.StatusCode HttpStatusCode.BadRequest "a POST without the CPF property should return 400"
    }

let postCpfNonStringCpf =
    task {
        use host = createHost().Start()
        let client = host.GetTestClient()
...
let postCpfSuccess =
    task {
        ...
        let body =
            JsonContent.Create({| cpf = "12345678910" |})

        let! response = client.PostAsync("/score", body)

        Expect.equal response.StatusCode HttpStatusCode.Created "a POST with a new, valid CPF should return 201"
    }
...
let getNonExistingCpf =
    task {
        ...
        Expect.equal response.StatusCode HttpStatusCode.NotFound "a GET with a non-existing CPF should return 404"
    }
